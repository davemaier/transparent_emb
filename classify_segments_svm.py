...
def load_svm_model(model_path: str) -> Dict[str, Any]:
    """Load the pretrained SVM model"""
    with open(model_path, 'rb') as f:
        model_data = pickle.load(f)

    print(f"Loaded SVM model trained on feature extraction run: {model_data['feature_extraction_model_run_id']}")
...
async def create_classification_model_run(
    ...
    name = f"SVM Classification {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    description = f"SVM classification of segments from segmentation run {seg_model_run_id}"

    metadata = {
        "seg_model_run_id": seg_model_run_id,
        "model_path": model_path,
        "model_type": "svm_classification",
...
async def classify_segments(
    ...
    """Main function to classify segments using the pretrained SVM"""
    ...
    try:
        ...
        print(f"Created SVM classification model run {classification_model_run_id}")
        ...
        prediction_probabilities = classifier.predict_proba(X_scaled)
        ...
        for i, item in enumerate(feature_data):
            ...
            prediction_records.append({
                ...
                'reason': f"SVM prediction with {confidence:.3f} confidence"
            })
        ...
        print(f"\nSVM classification completed! Model run ID: {classification_model_run_id}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Classify segments using pretrained SVM model"
    )
    parser.add_argument(
        "--seg-model-run-id",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Path to the pretrained SVM model file"
    )
    parser.add_argument(
        "--db-url",
